...
file_names = [
    "experiments/increments_experiment/outputs/initial/result_0.csv", 
    "experiments/increments_experiment/outputs/initial/result_1.csv", 
    "experiments/increments_experiment/outputs/initial/result_2.csv", 
    "experiments/increments_experiment/outputs/initial/result_3.csv", 
    "experiments/increments_experiment/outputs/initial/result_4.csv"
]
...
combined_averages_df.columns = ['Extra','Dataset Name', 'Increment', 'Orig_Acc', 'Eda_Acc', 'Aeda_Acc', 'Num_Acc', 'Alpha_Acc', 'Hybrid_Acc']
combined_averages_df = combined_averages_df.drop(columns=['Dataset Name'])
combined_averages_df.columns = ['Dataset', 'Increment', 'Orig_Acc', 'Eda_Acc', 'Aeda_Acc', 'Num_Acc', 'Alpha_Acc', 'Hybrid_Acc']
combined_averages_df = combined_averages_df.dropna()
# Save the combined dataframe to a CSV file
combined_averages_df.to_csv("experiments/increments_experiment/outputs/initial/aggregated_results.csv", index=False)
#print(combined_averages_df.dtypes)

# Group by 'Increment' and calculate the mean for each group
grouped = combined_averages_df.drop(columns=['Dataset']).groupby('Increment').mean()
...
metrics = ['Orig_Acc', 'Eda_Acc', 'Aeda_Acc', 'Num_Acc', 'Alpha_Acc', 'Hybrid_Acc']
for metric in metrics:
    plt.plot(grouped.index, grouped[metric], marker='o', label=metric)

plt.title('Average Metrics Across All Datasets Grouped by Increment')
...
placeholder_path = 'experiments/increments_experiment/outputs/initial/plots/average_metrics_across_increments.png'
